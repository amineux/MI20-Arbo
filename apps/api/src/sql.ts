...
const PG_KEYWORDS = new Set(
  `
    ...
    TRIM BTRIM LTRIM RTRIM UPPER LOWER LENGTH SUBSTR SUBSTRING POSITION OVERLAY
    CHAR_LENGTH CHARACTER_LENGTH ASCII CONCAT LPAD RPAD
  `
    .trim()
    .split(/\s+/)
    .filter(Boolean),
...
function quotePgIdentifiers(sql: string): string {
  return sql.replace(/"([^"]*)"|'([^']*)'|@\w+|:\w+|\$\d+|\b([A-Za-z_][A-Za-z0-9_]*)\b/g, (all, _dq, _sq, ident) => {
    ...
    // PascalCase columns (Id, GroupeLigne) need quotes; ALL-CAPS SQL functions must not
    // become "TRIM" / "UPPER" (Postgres then looks for a quoted function that does not exist).
    if (ident === ident.toLowerCase() || ident === ident.toUpperCase()) return ident;
    return `"${ident}"`;
  });
}

/** SQLite-flavored SQL → Postgres text (quoted identifiers, datetime, INSERT OR IGNORE). */
export function translateToPostgres(sql: string): string {
  return toPgSql(sql).text;
}
...
export function sqliteDdlToPostgres(stmt: string): string {
  let s = stmt.trim();
  ...
  const wasIgnore = /INSERT\s+OR\s+IGNORE/i.test(stmt);
  s = s.replace(/\bINSERT OR IGNORE INTO\b/gi, "INSERT INTO");
  s = quotePgIdentifiers(s);
  // Original SQLite text is `INSERT OR IGNORE INTO`, so do not require a literal
  // `INSERT INTO` match on `stmt` (that check dropped ON CONFLICT and broke remigrate).
  if (wasIgnore && !/\bON\s+CONFLICT\b/i.test(s)) {
